...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="dri:field[@type='composite'][dri:field/dri:instance | dri:params/@operations]" mode="formComposite" priority="2">
		...
		<!-- M.M. -->
                <xsl:choose>
                <xsl:when test="contains(@n, 'search-filter')">
                        <xsl:attribute name="value">Filter anwenden</xsl:attribute>
                        
                </xsl:when>     
                <xsl:otherwise>
                        <xsl:attribute name="value">Hinzufügen</xsl:attribute>
                </xsl:otherwise>
		<!-- M.M. ends -->
                </xsl:choose>
	...
	<!-- <xsl:apply-templates select="." mode="normalField"/> -->
        ...
        <xsl:if test="@id='aspect.discovery.SimpleSearch.field.query'">
                ...
                <xsl:apply-templates select="//dri:field[@id='aspect.discovery.SimpleSearch.field.submit']" mode="normalField"/>       
                 <input type="reset" value="Zurücksetzen" onclick="javascript:resetSearch();" name="reset" class="ds-button-field"/>
        </xsl:if>
