...
<xsl:stylesheet
	...
	exclude-result-prefixes="xalan encoder i18n dri mets dim  xlink xsl confman util atom ore oreatom">
	...
	<xsl:template match="dim:dim" mode="itemSummaryList-DIM-file">
		...
		<div class="item-metadata" style="width: {$metadataWidth}px;">
			...
			<xsl:if test="dim:field[@element='title' and @qualifier='alternative']">
				<span class="subtitle" style="width: {$metadataWidth - 110}px;">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
                        ...
                        <xsl:if test="dim:field[@element='relation' and @qualifier='ispartofseries']">
                                <span class="bold">In<xsl:text>:</xsl:text></span>
                                <span class="content" style="width: {$metadataWidth - 110}px;">
	...
	<xsl:template match="dim:dim" mode="itemSummaryList-DIM-metadata">
		...
		<div class="artifact-description">
			...
			<xsl:if test="dim:field[@element='identifier' and @qualifier='citation']">
				<xsl:if test="contains(dim:field[@element='identifier' and @qualifier='citation'], '(eds.)')">
	                                <span class="bold">In<xsl:text>:</xsl:text></span>
        	                        <span class="content">
                	                        <xsl:value-of select="substring-before(dim:field[@element='identifier' and @qualifier='citation'], ', p.')"/>
                        	        </span>
				</xsl:if>
                        </xsl:if>
			...
			<div class="artifact-preview">
				<a class="image-link" href="{$href}">
					<xsl:choose>
						...
						<xsl:otherwise>
							<img alt="Icon" src="{concat($theme-path, '/images/mime-small.png')}"/>
						</xsl:otherwise>
					</xsl:choose>
				</a>
			</div>
